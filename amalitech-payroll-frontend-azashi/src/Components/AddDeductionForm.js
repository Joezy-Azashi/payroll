...
import { InputAdornment, FormControl, Grid, MenuItem } from "@material-ui/core";
...
import { addDeductionsWithThunk, fetchDeductionsWithThunk } from "../Services/_redux/deductions/index"
import { deductionsReducer, handleStatusUpdate, handlePreloader } from "../Services/_redux/deductions/deduction_slice"
import moment from "moment";

function AddDeductionForm( { handleCloseForm }) {
...
const yearList = () => {
  ...
  const requestState = deductions.request
  ...
  const [amountDue, setTotalAmount] = useState("");
  const [monthlyDeduction, setMonthDeduction] = useState("");
...
const handleEndMonthChange = async (val) => {
  ...
  // FETCHING LIST OF DEDUCTION TAGS IN THE SYSTEM
  ...
  //   FUNCTION TO CREATE A DEDUCTION FORM
  const addDeductions = async (e) => {
    e.preventDefault();
    const DeductionCreate = []
    const duration = getMonthsDifference(startmonth, startyear, endmonth, endyear)
    if ((parseFloat(amountDue)) < (parseFloat(monthlyDeduction))){ //check if monthly bonus is not more than total bonus
      handleAlertState(true, "error", "Monthly Deduction cannot be more than total deduction");
      return
    }

    if ((duration * (parseFloat(monthlyDeduction))) !== (parseFloat(amountDue))){ //check if the product of the monthly bonus and the month duration is equal to total bonus
      handleAlertState(true, "error", "Monthly total of the monthly deduction does not tally with the specified total");
      return
    }
    ...
    employeeNumber.forEach((emp) => {
      ...
      DeductionCreate.push(deduct);
    });
    console.log("DeductionCreate", DeductionCreate)
    await dispatch(addDeductionsWithThunk(DeductionCreate))
    await dispatch(handleStatusUpdate({data: null}))
    await dispatch(fetchDeductionsWithThunk({page: 0}))
    closeForm()

  };
  const getDeductions = (lists) => {
      return lists.filter((list) => {
          return (list.tagType === "Deduction")
      })
  }
  ...
  useEffect(() => {
    if (deductions.status === 'Failed'){
        if(getErrorCode(deductions.error.error.message).includes('416')) {
            const error = {
                response: {
                    status: 416
                }
        ...
        else if (getErrorCode(deductions.error.error.message).includes('401')) {
            const error = {
                response: {
                    status: 401
                }
  ...
  return (
    <div>
      ...
      <div className="row mb-4">
            ...
            <div className="col-md-6">
                <FormControl fullWidth variant="outlined">
                  <InputLabel htmlFor="outlined-adornment-amount">Monthly Deduction</InputLabel>
                  <OutlinedInput
                  style={{height: "40px"}}
                    id="monthly_bonus"
      ...
      <Dialog
        open={deductions.request.preloader}
